...
def scheme_eval(expr, env, _=None):  # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        return scheme_apply(oper, oprnd, env)
        # END PROBLEM 4
...
def eval_all(expressions, env):
    ...
    if expressions is nil:
        ...
        while expressions is not nil:
            final_value = scheme_eval(expressions.first, env, True)
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        frame = Frame(self)
        while formals != nil:
            Frame.define(frame, formals.first, vals.first)
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 5
        target_value = scheme_eval(expressions.rest.first, env)
        ...
        new_pair = do_lambda_form(Pair(formals, body), env)
        env.define(name, new_pair)
        return name
        # END PROBLEM 9
    else:
...
def do_and_form(expressions, env):
    ...
    # BEGIN PROBLEM 12
    if expressions is nil:
        return is_true_primitive((expressions, env))
    while expressions is not nil:
